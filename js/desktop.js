const Desktop = {
  ...
  tick() {
    ...
    document.getElementById("lock-time").textContent = n.toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" });
    document.getElementById("lock-date").textContent = n.toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric" });
  },
  renderDock() {
    const dock = document.getElementById("dock");
    const items = [...this.pinned, ...this.extra.filter((x) => !this.pinned.includes(x))];
    dock.innerHTML = items.map((id, i) => {
      const sep = id === "aura" && i === items.length - 1 ? '<div class="dock-sep"></div>' : "";
      const icon = Icons[id] ? Icons[id]() : Icons.settings();
      return `${id === "settings" ? '<div class="dock-sep"></div>' : ""}
  ...
  search(q) {
    const keys = Object.keys(this.labels).filter((k) =>
      this.labels[k].toLowerCase().includes(q.toLowerCase())
    );
